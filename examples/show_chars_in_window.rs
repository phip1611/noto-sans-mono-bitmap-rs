...
use noto_sans_mono_bitmap::{get_bitmap_width, BitmapHeight, FontWeight, get_bitmap};
...
fn main() {
    ...
    for (char_i, char) in msg.chars().enumerate() {
        let letter_bitmap =
            get_bitmap(char, font_weight, bitmap_height);
        for (row_i, row) in letter_bitmap.iter().enumerate() {
            for (col_i, intensity) in row.iter().enumerate() {
                ...
                let index = char_i * char_width + col_i + row_i * buffer_width;
